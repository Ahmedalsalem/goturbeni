...
| `CRON_SECRET` | Yolculuk hatırlatma e-postası webhook'unu (`src/app/api/cron/departure-reminders`) doğrular — `send_departure_reminders()`'ın (`0081_departure_reminder.sql`) pg_net ile attığı isteğin `Authorization` başlığı bununla karşılaştırılır. Production'da Postgres'teki `app.cron_secret` GUC'sıyla (bkz. migration yorumu) birebir aynı olmalı; boşsa hatırlatma e-postaları hiç gönderilmez. |
