...
- **E-posta bildirimi için üçüncü taraf hesap gerekir**: `src/lib/email.ts` Resend kullanacak şekilde yazıldı ama `RESEND_API_KEY`/`RESEND_FROM_EMAIL` boşken no-op'a düşer — bir Resend hesabı açılıp Vercel'e eklenmeden gerçek e-posta gönderilmez (VAPID push ile aynı desen).
- **Yeni özelliklerin E2E testleri CI'da gerçek bir koşuyla doğrulanmadı**: `e2e/payment-review.spec.ts` (dekont red/onay, kalan ödeme, coğrafi yakın-il araması) bu depoda Docker/WSL olmadığı için yalnızca statik olarak yazıldı/gözden geçirildi, push sonrası CI'da izlenmesi gerekiyor — bkz. PROJECT_STATUS.md → Faz 12.
