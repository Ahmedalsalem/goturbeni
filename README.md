...
- **Kapora dekontu OCR ile otomatik onay**: yolcu kapora dekontunu yüklediğinde, sunucu tarafında Tesseract (`tesseract.js`, ücretsiz/açık kaynak — `src/lib/ocr.ts`) dekonttan IBAN ve tutar adaylarını okur; bunlar `submit_deposit_receipt_ocr` (`0053_deposit_ocr_auto_approval.sql`) RPC'sinde sürücünün gerçek kayıtlı IBAN'ı ve ilanın gerçek kapora tutarıyla (masraf payı × koltuk × 0.5, ±5₺ tolerans) **veritabanı içinde yeniden karşılaştırılır** — istemciden/server action'dan gelen bir "eşleşti" bayrağına asla güvenilmez. Eşleşme, hesap ≥14 gün yaşında/askıya alınmamış/bu rezervasyonda geçmiş dekont reddi yok/şüpheli hesap değil/açık anlaşmazlığı yok (aynı risk kriterleri, `0047`) koşuluyla birleşince rezervasyon **sürücü hiçbir şey yapmadan** otomatik onaylanır — sürücünün manuel "İlk Yarı Ödemesini Aldım, Onayla" butonu eşleşmeyen/yüksek riskli durumlar için yedek olarak aynen kalır. OCR görüntü-metin okumasıdır, gerçek banka doğrulaması değildir — doğru IBAN/tutarı bir görsele yazmak mümkün olduğundan bu bugünkü sıfır-doğrulamalı sürücü self-approve'undan daha güçlü bir sinyal, ama kesin kanıt değildir (bkz. Bilinen Sınırlamalar).
