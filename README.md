...
- **Kimlik doğrulama ve hesap onayı**: E-posta/şifre ile kayıt ve giriş, **veya Google ile tek tıkla giriş** (Supabase Auth OAuth, `signInWithGoogle`) — ilk Google girişinde `handle_new_user` tetikleyicisi ad-soyad/avatarı Google'ın profilinden otomatik yazar ve hesap baştan doğrulanmış sayılır (`0078_google_oauth_signup.sql`, Google zaten e-posta sahipliğini kanıtlamıştır). Parola ile kayıtlarda zorunlu hesap doğrulaması, 6 haneli bir kodun Resend ile e-postaya gönderilip `verify_email_otp` RPC'siyle onaylanmasına dayanır (`0035_email_based_verification.sql`). **Telefon numarası doğrulama yükü taşımaz** — yalnızca iletişim/güven bilgisi olarak kayıt sırasında toplanıp saklanır; eskiden (Twilio Verify ile) SMS/OTP tabanlıydı, Twilio'nun ücretsiz deneme hesabı yalnızca önceden doğrulanmış numaralara SMS gönderebildiğinden ve gerçek SMS gönderiminin operatör maliyeti olduğundan bu model terk edildi (bkz. CHANGELOG). Doğrulama sonucu `profiles_private.email_verified` sütununda tutulur (`0077_rename_phone_verified_to_email_verified.sql` — eski `phone_verified` adı isim değişikliğinden önce hep yanıltıcıydı).
- **Profil yönetimi**: Ad, telefon, biyografi, cinsiyet (kendi beyanına dayalı, kimlik doğrulamalı değil), araç marka/modeli/plakası/rengi düzenleme; Supabase Storage üzerinden avatar yükleme (kullanıcı başına izole klasör). Plaka, girildiğinde standart Türk plaka formatına göre doğrulanır (uygulama katmanında Zod, veritabanında `NOT VALID` bir check constraint, `0051_car_plate_format_check.sql`) ve bir ilan yayınlayabilmek (ya da yolcu ilanına teklif verebilmek, `0079`) için sürücü profilinde geçerli formatta bir plaka **ve** bir araç rengi bulunması **zorunludur** (IBAN kontrolüyle aynı desende, `features/rides/actions.ts` → `createRide`, `0076_car_color.sql`) — yolcunun doğru aracı teşhis edebilmesi için.
- **İlan sistemi**: İlan oluşturma/düzenleme/iptal etme, 81 il + opsiyonel ilçe arasından kalkış/varış seçimi, tarih/saat/koltuk/masraf payı/açıklama alanları, evcil hayvan/sigara/klima tercihleri, VIP (tek yolcu) ilan seçeneği, ödeme yöntemi seçimi (banka havalesi/nakit, yalnızca sürücü ilanında — bkz. "Ödeme takibi"), anında onay seçeneği (bkz. "Rezervasyon sistemi"). Sürücü ilanında "Ücretsiz yolculuk" kutusu masraf payını 0'a kilitler (ilan kartında/detayda "0 ₺" yerine "Ücretsiz" gösterilir) — yolcu ilanında bu seçenek yok, orada masraf payı 0'dan büyük olmak zorundadır (kimseden bedava yolculuk istenemez). İlk kez ilan veren, ne kadar masraf payı isteyeceğini bilmeyen bir sürücü için: kalkış/varış/koltuk sayısı girilince, iki il arası kuş uçuşu mesafeye (`turkish-provinces-geo.ts`) dayalı kaba bir tahmin ("Tahmini maliyet: ~X ₺") ve tek tıkla o tutarı doldurma butonu gösterilir (`src/utils/cost-estimate.ts`) — asla otomatik doldurmaz, yalnızca öneridir; ₺/km oranı sabit bir varsayımdır, canlı yakıt fiyatı çekmez. Yolcu ilanında, sürücünün "araç özellikleri" checkbox listesi (klima, USB şarj, geniş bagaj vb.) artık yolcunun kendi ihtiyaçlarını ("İhtiyaçlarım") işaretlemesi için de kullanılabiliyor.
...
- **SEO rota/şehir sayfaları (programmatic SEO)**: `/istanbul-ankara-arac-paylasimi` gibi 10 popüler rota + `/istanbul-arac-paylasimi` gibi 4 şehir sayfası, tek bir kök-seviye `src/app/[slug]/page.tsx` catch-all route ile (`src/features/seo/route-pages.ts`'teki sabit slug listesi, `generateStaticParams`). Sitenin geri kalanı canonical mekanizması (`x-pathname` middleware header) yüzünden zorunlu dinamik iken, bu sayfalar **statik/SSG** üretilir — canonical doğrudan bilinen slug'tan kurulur, `headers()`'a hiç dokunulmaz. İçerik tamamen gerçek: mesafe (`turkish-provinces-geo.ts`), maliyet tahmini (`cost-estimate.ts`), canlı ilanlar (`getRides`) — sahte veri yok, ilan yoksa dürüst bir "aktif ilan yok" mesajı ve tek CTA ("Bu güzergahta ilan ver") gösterilir. Her sayfada `BreadcrumbList` JSON-LD ve ilgili rota/şehir sayfalarına internal link vardır. Ana sayfaya da gerçek, crawl edilebilir linklerle bir "Popüler Rotalar" bölümü eklendi (`HomePopularRoutes.tsx`).
...
- **Çok dilli ve RTL**: Türkçe / Arapça / İngilizce arayüz (`messages/tr.json`, `messages/ar.json`, `messages/en.json` — 919 anahtar birebir eşleşir), cookie tabanlı locale seçimi, Arapça için tam RTL desteği (yön ikonları dahil).
...
e2e/                            # Playwright uçtan uca testleri — 13 dosya: booking-chat-review, double-booking, payment-review, new-features, passenger-listing, instant-booking, cash-payment, settlement-ocr-auto-approval, disputes, pickup-verification, bulk-receipt-approval, fraud-detection, cost-estimate
public/
  sw.js                        # temel offline fallback service worker
  icons/, apple-touch-icon.png # PWA ikonları
supabase/
  migrations/                  # 0001–0079, bkz. "Migration Sırası"
```
...
| `CRON_SECRET` | Yolculuk hatırlatma e-postası webhook'unu (`src/app/api/cron/departure-reminders`) doğrular — `send_departure_reminders()`'ın (`0082_departure_reminder_config_table.sql`) pg_net ile attığı isteğin `Authorization` başlığı bununla karşılaştırılır. Production'daki `public.cron_config` tablosunun `cron_secret` satırıyla birebir aynı olmalı; boşsa hatırlatma e-postaları hiç gönderilmez. |
...
52. `supabase/migrations/0068_admin_user_emails.sql` — admin kullanıcı listesinde e-posta gösterilebilsin diye `admin_get_user_emails` RPC'si (`auth.users` PostgREST'ten okunamadığından, `0025`'teki desenle admin-gated security-definer, yalnızca istenen `p_user_ids` scope'unda).
53. `supabase/migrations/0069_signup_full_name_required.sql` — kayıt formuna ad-soyad alanı eklendi (önceden yoktu, admin panelde bir sürü kullanıcı "Kullanıcı" görünüyordu); `complete_registration_details`'a `p_full_name` eklendi.
54. `supabase/migrations/0070_car_features.sql` — tek "Klimalı" checkbox'ı yerine profilde ~10 sabit araç özelliği (`car_features`) + serbest metin özel özellikler (`custom_car_features`); eski `has_ac` kaldırıldı.
55. `supabase/migrations/0071_ride_car_features.sql` — araç özellikleri profil yerine ilana taşındı (`rides.car_features`/`custom_car_features`) — "bu seferki araçta klima yok" gibi ilan bazlı istisnalara izin vermek için.
56. `supabase/migrations/0072_ride_extra_preferences.sql` — `pets_allowed`/`smoking_allowed`/`vip_solo` yanına üç yeni yolculuk tercihi: `quiet_ride`, `no_large_luggage`, `no_stops`.
57. `supabase/migrations/0073_ride_payment_methods_multi.sql` — ödeme yöntemi artık tekil enum değil dizi (`payment_methods`) — sürücü hem havale hem nakit kabul edebiliyor.
58. `supabase/migrations/0074_rides_update_grant_drift_fix.sql` — `0058`'in dinamik "grant update" çalıştırması `0065`/`0070`-`0073`'ün eklediği yeni kolonları hiç kapsamamıştı (bakım notuna rağmen atlanmıştı) — `updateRide` bu kolonları sessizce güncelleyemiyordu, grant listesi tamamlanarak düzeltildi.
59. `supabase/migrations/0075_broadcast_optout_age_gate_account_deletion.sql` — üç bağımsız madde: yeni-ilan yayın e-postasına `email_notifications_enabled` filtresi (artık gerçek bir opt-out var), kayıt formuna 18 yaş altı reddeden doğum tarihi alanı, kendi kendine hesap silme (`delete_own_account`, anonimleştirme).
60. `supabase/migrations/0076_car_color.sql` — profile `car_color` eklendi, `car_plate` ile aynı desende (ilan verebilmek için sürücü profilinde zorunlu).
...
69. `supabase/migrations/0085_ride_more_preferences.sql` — ilan formundaki "Araç Özellikleri" checklist'i tamamen kaldırılıp yerine `large_luggage_ok`/`child_seat_available`/`wheelchair_accessible` boolean tercihleri eklendi (`rides`+`ride_series`), `generate_recurring_rides()` güncellendi, `rides`'ın dinamik update grant'ı (`0058`/`0074` deseni) yeniden verildi. Tüm tercihler artık hem sürücü hem yolcu ilanında, role-conditional metinle (örn. "Sigara içilebilir" / "Sigara içiyorum") kullanılabiliyor.
70. `supabase/migrations/0086_ride_usb_ac_admin_resend.sql` — `usb_charger_available`/`ac_available` aynı desende eklendi (form+kart+arama filtresi), ve `admin_resend_verification_code` RPC'si: adminin doğrulanmamış hesaplara 6 haneli, **süresiz** (`9999-12-31`) yeni bir kod atayıp özür içeren ayrı bir e-posta (`sendAdminResendVerificationEmail`) gönderebilmesi için.
