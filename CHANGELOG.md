...
- **E2E testleri**: yeni `e2e/payment-review.spec.ts` — dekont red/gerekçe/yeniden yükleme/onay, admin IBAN gösterimi, kalan ödeme dekontu, ve coğrafi yakın-il arama fallback'i uçtan uca doğruluyor. Ayrıca **mevcut iki spec'te bulunup düzeltilen bir regresyon**: bu oturumdan önceki bir oturumda eklenen zorunlu telefon OTP doğrulaması (`requireVerifiedProfile`, ilan/rezervasyon oluşturmayı engelliyor) ve rezervasyon onay butonunun metninin değişmesi ("Onayla" → "İlk Yarı Ödemesini Aldım, Onayla"), test yerelde/CI'da hiç çalıştırılmadığı için fark edilmeden `e2e/booking-chat-review.spec.ts` ve `e2e/double-booking.spec.ts`'i kırmıştı — `e2e/utils.ts`'e servis-role ile telefonu doğrulayan bir test-bypass'i (`verifyPhoneForTest`/`signUpAndVerify`, `backdateRideDeparture` ile aynı desen) eklenip her iki spec güncellendi. **Bu depoda Docker/WSL olmadığından hiçbiri bu oturumda gerçek Supabase'e karşı çalıştırılmadı** — yalnızca statik olarak (kaynak koddaki gerçek metin/davranışla karşılaştırılarak) doğrulandı; push sonrası CI'da gerçek bir koşuyla doğrulanması gerekiyor (önceki oturumların yaptığı gibi, bkz. PROJECT_STATUS.md → Faz 11).
- Bu belge, PROJECT_STATUS.md ve README.md güncellendi.

Doğrulama (bu oturumda): `npm run lint` (temiz), `npx tsc --noEmit` (temiz, `e2e/` dahil), `npm run build` (34 rota, başarılı), `npm test` (114/114 geçti), i18n anahtar eşleşmesi (tr/ar, 594/594 birebir). **E2E testleri CI'da çalıştırılıp izlenmedi** (bkz. yukarı) — bu, "done" değil "implemented, unverified" olarak işaretlenmesi gereken tek parça.
