...
- **Otomatik E2E testleri**: kullanıcı, CI'da local (Docker tabanlı) Supabase ile tam entegre bir kurulumu tercih etti. `playwright.config.ts` (`npm run dev`'e karşı, tek chromium projesi), `e2e/utils.ts` (kayıt/giriş, Base UI Combobox sürücüsü, ilan oluşturma, service-role ile `departure_time` geriye alma), `e2e/booking-chat-review.spec.ts` (kayıt → ilan oluşturma → filtrelerle bulma → rezervasyon talebi/onayı → realtime sohbet → kalkış sonrası karşılıklı değerlendirme, iki kalıcı browser context'iyle) ve `e2e/double-booking.spec.ts` (son koltuk için iki talep, ikinci onayın RPC tarafından reddedildiğinin doğrulanması, `0003_bookings.sql`) eklendi. `.github/workflows/ci.yml`'e mevcut lint/typecheck/build/unit-test job'ını yavaşlatmayan, ayrı ve paralel bir `e2e-tests` job'ı eklendi: `npx supabase start` ile local (gerçek linked projeye hiç dokunmayan) bir instance başlatıyor, kimlik bilgilerini `supabase status -o json`'dan `$GITHUB_ENV`'e yazıyor, yalnızca chromium kuruyor, `npm run test:e2e` çalıştırıyor. **Bu oturumda gerçek bir local Supabase'e karşı çalıştırılıp doğrulanamadı** — ne subagent'ın sandbox'ında ne de ana oturumda Docker veya WSL Linux dağıtımı mevcuttu (`docker: command not found`, `wsl -l -v` dağıtım yok diyor). Spec'lerdeki tüm alan id'leri, buton/toast metinleri (`messages/tr.json`) ve RPC/RLS semantiği gerçek kaynak koduna karşı elle doğrulandı ve `npx tsc --noEmit`/`npm run lint` temiz geçti, ama fiilen çalıştırılıp yeşil göründükleri hiç gözlemlenmedi. GitHub'ın `ubuntu-latest` runner'larında Docker önceden kurulu olduğundan, gerçek ilk doğrulama bu commit push edildiğinde CI'da olacak — Actions sekmesinden `e2e-tests` job'ının sonucu kontrol edilmeli.

Doğrulama (bu oturumda): `npm run lint` (temiz), `npx tsc --noEmit` (temiz, e2e/ dahil), `npm test` (110/110 geçti — yeni eklenen rate-limit fail-fast testi dahil, e2e/ specs'i vitest'e dahil değil), `npm run build` (başarılı, Upstash env değişkenleri olmadan da — fail-fast kontrolü `next build`'in sayfa verisi toplama adımını kırmayacak şekilde lazy yapıldı, bu ayrıca doğrulandı).
