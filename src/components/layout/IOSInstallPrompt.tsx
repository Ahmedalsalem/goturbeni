...
// All iOS browsers (Chrome, Firefox, Edge, Opera) are WebKit wrappers around
// Safari and share its UA string, but only Safari's own Share sheet actually
// installs a home-screen PWA — the others' "Add to Home Screen" just makes a
// bookmark. Detect the non-Safari wrappers by their own UA tokens so those
// visitors get told to switch browsers instead of a guide that won't work.
...
export function IOSInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (isIosBrowserTab()) {
      setVisible(true)
      setNeedsSafari(isNonSafariIosBrowser())
    }
  }, [])
    ...
    >
      <div className="pointer-events-auto mx-auto flex max-w-6xl items-start gap-3 sm:max-w-md sm:items-center">
        <div className="flex-1 text-sm">
          ...
          {needsSafari ? (
            <p className="text-muted-foreground">{t("openInSafari")}</p>
          ) : (
