  ...
  return (
    z
      .object({
        ...
        paymentMethods: z
          .array(z.enum(["bank_transfer", "cash"]))
          .min(1, t("paymentMethodRequired"))
      ...
      .refine(
        (data) => !data.arrivalDistrict || TURKISH_PROVINCE_DISTRICTS[data.arrivalCity]?.includes(data.arrivalDistrict),
        {
          message: t("districtInvalid"),
          path: ["arrivalDistrict"],
        }
      )
      // Anında onay/haftalık tekrar yalnızca sürücü için anlamlı (ilan sahibi
      // henüz sürücü değil) — form bunları zaten gizliyor, ama şema seviyesinde
      // de zorlanıyor ki tamperlenmiş bir istek bu alanları dolaylı yoldan set
      // edemesin. Ödeme yöntemi artık istisna: yolcu ilanında da hangi ödeme
      // şeklini kullanabileceğini belirtir (driver_id ilan onaylanınca
      // atanıyor, o noktadan sonra ride.payment_methods aynı bank_transfer/cash
      // ayrım mantığıyla kullanılıyor — bkz. /rides/[id]/bookings). petsAllowed/
      // smokingAllowed/largeLuggageOk/childSeatAvailable/wheelchairAccessible/
      // usbChargerAvailable/acAvailable de aynı sebeple sıfırlanmaz: yolcu için
      // "kendi ihtiyacım" anlamına döner (ör. "sigara içiyorum").
      .transform((data) =>
